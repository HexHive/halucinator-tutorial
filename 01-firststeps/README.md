...
In this part of the tutorial we will run, for the first time, a firmware under HALucinator. 
...
Our first step is to inspect the mem.yaml file. It should look like this:
...
We need to tell HALucinator what firmware to use. To do this, replace `stm32-firmware-here` with 
the name of the firmware file.

This MUST be the flat binary format that would be written to flash. In this case, 
it is the BIN file and not the ELF file, so replace this with `stm32-f411re-uart.bin`.

Next, open `config.yaml`. You will see that some interception points have already been 
added for you. However, not all are present. Copy and paste the following UART-specific 
interception points into the file:
